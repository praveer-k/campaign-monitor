...
app.controller('searchCtrl', function($scope, $http, common){
  ...
  $scope.search = function(keyword){
    ...
    $http.get('/api/search/'+keyword).success(function(response){
  ...
  $scope.$watch(function(){
    ...
    if(common.variables.filepath!='/output/intro.html' && common.variables.filepath!=''){
      ...
      if($scope.objs!=common.variables.jsonData){
        ...
        if($scope.objs.length!=0){ common.paginate($scope); }else{ $scope.objs=undefined; }
      }
    }
  });
...
app.controller('listOfReports', function($scope, $http, common, $location, $anchorScroll){
    ...
    $scope.load = function(obj){
      common.variables.filepath = '/output/'+obj.dbname+'/'+obj.dbname+'_report.html';
      ...
      common.variables.outfile3 = '/output/'+obj.dbname+'/'+obj.dbname+'_report.pdf';
    ...
    return {
      ...
      link : function($scope, element, attrs, ngModel) {
        $scope.$watch(attrs.ngModel, function(value) {
          $http.get('/api/db/'+ value).success(function(response){
